...
- **Feature Flags** — `IFeatureManager` / `TFeatureManager` reads `FeatureManagement:*` live from configuration; host enables JSON/YAML **ReloadOnChange**; `TFeatureConfiguration.Reload` forces `IConfigurationRoot.Reload` (S68).
