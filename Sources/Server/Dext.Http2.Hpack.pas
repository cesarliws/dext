...
{  Implements the static table (61 entries), a FIFO ring-buffer dynamic    }
{  table, Huffman decoding (FSM-based), and header encode/decode.          }
{                                                                           }
{***************************************************************************}
unit Dext.Http2.Hpack;
...
const
  /// <summary>Number of pre-defined entries in the HPACK static table (RFC 7541 Appendix A).</summary>
  HPACK_STATIC_TABLE_SIZE = 61;

  /// <summary>Default HPACK dynamic table maximum size in bytes (SETTINGS_HEADER_TABLE_SIZE).</summary>
  HPACK_DEFAULT_TABLE_SIZE = 4096;

  /// <summary>Maximum number of entries allowed in the dynamic table ring buffer.</summary>
  HPACK_MAX_DYNAMIC_ENTRIES = 256;

type
  /// <summary>A name/value pair representing a single HTTP/2 header field.</summary>
  TNameValuePair = record
    Name: string;
    Value: string;
  end;

  /// <summary>Array of name/value header pairs.</summary>
  TNameValuePairs = array of TNameValuePair;

  /// <summary>
  ///   Encapsulates a HPACK dynamic table using a FIFO ring buffer.
  ///   Entries are evicted oldest-first when the total encoded size would exceed MaxSize.
  ///   Encoded size of an entry = Length(Name) + Length(Value) + 32 (RFC 7541 §4.1).
  /// </summary>
  THpackDynamicTable = record
  private
    FEntries: array[0..HPACK_MAX_DYNAMIC_ENTRIES - 1] of TNameValuePair;
    FHead: Integer;     // index of oldest entry
  ...
  public
    ...
    /// <summary>Inserts a new header entry at the front of the dynamic table (index 62).</summary>
    procedure Add(const AName, AValue: string);
    /// <summary>Returns the entry at the given 1-based dynamic-table index (1 = newest).</summary>
    function Get(AIndex: Integer): TNameValuePair;
    /// <summary>Returns the number of entries currently in the dynamic table.</summary>
    function Count: Integer;
    /// <summary>Updates the maximum size, evicting entries as required.</summary>
    procedure SetMaxSize(ANewMaxSize: Integer);
    /// <summary>Returns the current total encoded byte size of all entries.</summary>
    property CurrentSize: Integer read FCurrentSize;
    /// <summary>Returns the configured maximum byte size.</summary>
    property MaxSize: Integer read FMaxSize;
  end;

  /// <summary>
  ///   Provides Huffman decoding for HPACK string literals (RFC 7541 Appendix B).
  ///   Uses a compact FSM table (2-level, 512 states) for O(n/8) throughput.
  /// </summary>
  THpackHuffman = class
  public
    /// <summary>Decodes a Huffman-encoded byte sequence to a UTF-8 string.</summary>
    /// <param name="AData">Pointer to the first byte of Huffman-encoded data.</param>
    /// <param name="ALength">Number of bytes to decode.</param>
    /// <returns>The decoded string value.</returns>
    class function Decode(AData: PByte; ALength: Integer): string; static;
    /// <summary>Encodes a UTF-8 string to Huffman-coded bytes.</summary>
    /// <param name="AValue">The string to encode.</param>
    /// <returns>The Huffman-encoded byte sequence.</returns>
    class function Encode(const AValue: string): TBytes; static;
    /// <summary>Returns the number of bits required to Huffman-encode the given string.</summary>
    class function EncodedBitLength(const AValue: string): Integer; static;
  end;

  /// <summary>
  ///   Decodes HPACK-compressed header blocks to name/value pairs (RFC 7541).
  ///   Supports Indexed Header Field, Literal with/without indexing, and dynamic table updates.
  /// </summary>
  THpackDecoder = class
  private
  ...
  public
    /// <summary>Initializes the decoder with the given maximum dynamic table size.</summary>
    constructor Create(AMaxTableSize: Integer = HPACK_DEFAULT_TABLE_SIZE);

    /// <summary>
    ///   Decodes a complete HPACK header block into name/value pairs.
    /// </summary>
    /// <param name="AData">Pointer to the beginning of the header block.</param>
    /// <param name="ALength">Length of the header block in bytes.</param>
    /// <param name="AHeaders">Output array that receives the decoded header pairs.</param>
    /// <returns>True on success; False if the header block is malformed.</returns>
    function Decode(AData: PByte; ALength: Integer;
      out AHeaders: TNameValuePairs): Boolean;

    /// <summary>Updates the maximum dynamic table size (from SETTINGS_HEADER_TABLE_SIZE).</summary>
    procedure SetMaxTableSize(ASize: Integer);
  end;

  /// <summary>
  ///   Encodes header name/value pairs to a HPACK-compressed header block (RFC 7541).
  ///   V1 uses Literal Header Field without Indexing (no Huffman on encode path).
  ///   The decoder fully supports Huffman-encoded input from clients.
  /// </summary>
  THpackEncoder = class
  private
    FDynTable: THpackDynamicTable;
  ...
  public
    /// <summary>Initializes the encoder with the given maximum dynamic table size.</summary>
    constructor Create(AMaxTableSize: Integer = HPACK_DEFAULT_TABLE_SIZE);

    /// <summary>
    ///   Encodes an array of name/value header pairs into a HPACK header block.
    /// </summary>
    /// <param name="AHeaders">The headers to encode.</param>
    /// <returns>The encoded header block bytes.</returns>
    function Encode(const AHeaders: TNameValuePairs): TBytes;

    /// <summary>Updates the maximum dynamic table size sent to the peer.</summary>
    procedure SetMaxTableSize(ASize: Integer);
  end;
...
//  We use a two-level 8-bit FSM: level-1 state × input-byte → (sym, newstate, bits)
//  For compactness we compute on-the-fly using the canonical bit-lengths.
// ---------------------------------------------------------------------------

//  Huffman code table: index = ASCII code, value = (code, bit-length)
type
  THuffEntry = record
    Code: Cardinal;
    Bits: Byte;
...
const
  HUFF_TABLE: array[0..257] of THuffEntry = (
    (Code: $1ff8;     Bits: 13),  // 0
    ...
    (Code: $ffffea;   Bits: 24),  // 9  HT
    (Code: $3ffffffc; Bits: 30),  // 10 LF
    (Code: $fffffe9;  Bits: 28),  // 11
    (Code: $fffffea;  Bits: 28),  // 12
    (Code: $3ffffffd; Bits: 30),  // 13 CR
    ...
    (Code: $7fff0;    Bits: 19),  // 92 backslash
    ...
    (Code: $23;       Bits:  6),  // 98 b
    (Code: $4;        Bits:  5),  // 99 c
    (Code: $24;       Bits:  6),  // 100 d
    (Code: $5;        Bits:  5),  // 101 e
    (Code: $25;       Bits:  6),  // 102 f
    (Code: $26;       Bits:  6),  // 103 g
    (Code: $27;       Bits:  6),  // 104 h
    (Code: $6;        Bits:  5),  // 105 i
    (Code: $74;       Bits:  7),  // 106 j
    (Code: $75;       Bits:  7),  // 107 k
    (Code: $28;       Bits:  6),  // 108 l
    (Code: $29;       Bits:  6),  // 109 m
    (Code: $2a;       Bits:  6),  // 110 n
    (Code: $7;        Bits:  5),  // 111 o
    (Code: $2b;       Bits:  6),  // 112 p
    (Code: $76;       Bits:  7),  // 113 q
    (Code: $2c;       Bits:  6),  // 114 r
    (Code: $8;        Bits:  5),  // 115 s
    (Code: $9;        Bits:  5),  // 116 t
    (Code: $2d;       Bits:  6),  // 117 u
    (Code: $77;       Bits:  7),  // 118 v
    (Code: $78;       Bits:  7),  // 119 w
    (Code: $79;       Bits:  7),  // 120 x
    (Code: $7a;       Bits:  7),  // 121 y
    (Code: $7b;       Bits:  7),  // 122 z
    (Code: $7ffe;     Bits: 15),  // 123 {
    (Code: $7fc;      Bits: 11),  // 124 |
    (Code: $3ffd;     Bits: 14),  // 125 }
    (Code: $1ffd;     Bits: 13),  // 126 ~
    (Code: $ffffffc;  Bits: 28),  // 127 DEL
...
begin
  // Evict oldest entry (at FHead position shifted by FCount-1)
...
begin
  ...
  while (FCurrentSize + size > FMaxSize) and (FCount > 0) do
    Evict;
  // If single entry exceeds limit, don't insert (RFC 7541 §4.4)
...
begin
  // AIndex is 1-based, newest first
...
begin
  ...
  FillChar(Result[0], byteLen, $FF); // pad with 1-bits (EOS prefix)
  ...
  begin
    ...
    // Push codeBits bits of code into our accumulator
  ...
  if bitsAvail > 0 then
  begin
    // Pad remaining with 1-bits (already set by FillChar)
...
begin
  ...
  bits := 0;
  bitsAvail := 0;
  SetLength(resultBytes, ALength * 2); // generous upper bound
  count := 0;

  // Load all bits into the accumulator byte-by-byte and decode
  for i := 0 to ALength - 1 do
  begin
    bits := (bits shl 8) or AData[i];
    Inc(bitsAvail, 8);

    // Greedily decode symbols while we have enough bits
    ...
    begin
      ...
      // Scan all symbols for the longest prefix match
      for sym := 0 to 255 do
      begin
        codeBits := HUFF_TABLE[sym].Bits;
        if codeBits > bitsAvail then Continue;
        code := HUFF_TABLE[sym].Code;
        // Extract top codeBits bits from accumulator
...
begin
  ...
  begin
    ...
    begin
      ...
      if index = 0 then Exit; // Index 0 is invalid
    ...
    begin
      ...
      Inc(pos, strRead);
      Dec(avail, strRead);
      // Never add to dynamic table (§6.2.2 / §6.2.3)
      SetLength(AHeaders, headerCount + 1);
      AHeaders[headerCount].Name := nameStr;
      AHeaders[headerCount].Value := valueStr;
      Inc(headerCount);
...
begin
  ...
  begin
    ...
    begin
      // Literal with Incremental Indexing, name as string (prefix 01, index=0)
